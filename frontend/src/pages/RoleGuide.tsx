...
const ROLES: RoleSection[] = [
  ...
  },
  {
    ...
    flow: [
      ...
      { title: "Anyone joined? → ask the director to create the account", title_id: "Ada yang masuk? → minta direktur membuatkan akun",
        detail: "Admin → Users is director-only; you file the documents once the account exists.",
        detail_id: "Admin → Pengguna hanya untuk direktur; Anda mengarsipkan dokumen setelah akunnya ada." },
  ...
  },
  {
    ...
    flow: [
      ...
      { title: "Money lands → verify the claim OR record it manually", title_id: "Uang masuk → verifikasi klaim ATAU catat manual",
        detail: "Portal customers file claims you verify. Customers who pay by transfer without the portal: 'Record a payment manually' on Payment verification, or 'Enter payment manually' on the project's invoice card — recorded + verified in one stroke. Full payment auto-closes the project.",
        detail_id: "Pelanggan portal mengirim klaim yang Anda verifikasi. Pelanggan yang transfer tanpa portal: 'Catat pembayaran manual' di Verifikasi Pembayaran, atau 'Masukkan pembayaran manual' di kartu faktur proyek — tercatat + terverifikasi sekaligus. Pembayaran penuh otomatis menutup proyek." },
    ...
    buttons: [
      ...
      { page: "Payment verification", page_id: "Verifikasi pembayaran", button: "Record a payment manually", button_id: "Catat pembayaran manual",
        effect: "For customers who paid by transfer without the portal — pick the open invoice, enter the amount, done (recorded + verified in one stroke)",
        effect_id: "Untuk pelanggan yang transfer tanpa portal — pilih faktur terbuka, isi jumlah, selesai (tercatat + terverifikasi sekaligus)" },
      { page: "Project detail (invoice card)", page_id: "Detail Proyek (kartu faktur)", button: "Enter payment manually", button_id: "Masukkan pembayaran manual",
        effect: "Same manual record-and-verify, right on the project's invoice",
        effect_id: "Pencatatan + verifikasi manual yang sama, langsung di faktur proyek" },
    ...
    rules: [
      ...
      "Two ways money gets recorded: portal customers file a claim you Verify; everyone else you enter yourself with 'Record a payment manually' — one stroke, already verified. Fully paying an invoice auto-advances its project to paid → closed.",
    ...
    rules_id: [
      ...
      "Dua cara uang tercatat: pelanggan portal mengirim klaim yang Anda Verifikasi; selain itu Anda masukkan sendiri lewat 'Catat pembayaran manual' — sekali jalan, langsung terverifikasi. Faktur lunas otomatis memajukan proyeknya ke lunas → tutup.",
...
const TROUBLES: { problem: string; problem_id: string; answer: string; answer_id: string }[] = [
  ...
  },
  {
    ...
    answer: "Finance (or Director) → Payment verification. If the customer paid by transfer without the portal, finance can skip the claim entirely with 'Record a payment manually'.",
    answer_id: "Keuangan (atau Direktur) → Verifikasi pembayaran. Kalau pelanggan transfer tanpa portal, keuangan bisa lewati klaim dengan 'Catat pembayaran manual'.",
  },
  {

...
- **An approved request can still be put in front of a supplier.** The vendor list used to offer only requests still waiting on a cost, which missed the case that happens most: the approved request is the one you actually buy against. The deal is won and the goods have to be ordered; the quote behind the cost has expired; the order itself changed. Approved requests now appear in **Ask suppliers for a price**, labelled *already priced*, alongside the ones still waiting (those sort first). Drafts and rejected requests stay out — one is still being written, the other is a dead deal.
- **Applying a quote to an approved request does not move the cost.** The margin was approved against the old number, so the quote is filed as a **cost revision** in the director's queue instead — the same entry in the revision log, the same approval row, the same button, as if purchasing had proposed it by hand. It used to be skipped silently, which looked exactly like a broken button. One pending revision at a time, so a second quote arriving while the first is still waiting is refused with a reason.
- The PR links forward: PR → quotation → project, all clickable on each other's pages, and the quotation links back with **View price request**.
...
The customer side is linked the same way. For sales, managers and the director, a price request lists **every quotation made from it** — revisions included, newest version first — with number, status and total, and the header's **Open quotation** button goes to the newest live one. The price-request list has a **Quotation** column showing that quotation, which opens it without opening the request. (Before, the only link followed the *first* quotation, so after a revision it led to the superseded R1.) Purchasing gets no quotation links: they are customer-blind and have no quotations page.
...
- **One invoice per project.** A project is billed once: one final (or single) invoice, and at most one down-payment invoice beside it — the deposit it balances. A second is refused with the number of the one already there, and there is no override; a rejected invoice does not count, so it can be replaced. The Issue buttons on the project page grey out once the project has its invoice. If one is wrong, delete it and issue it again.

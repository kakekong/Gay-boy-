    ...
    return (
      <div className="card p-12 text-center">
        ...
        <div className="text-sm muted mt-1">
          {T("This page shows every file uploaded across the system. Ask the director to grant access.")}</div>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2">
          <FileText size={22} className="text-brand-600" /> {T("All uploaded files")}</h1>
        <p className="text-sm muted">
          {T("Every drawing, invoice, signed PO, and proof-of-payment uploaded by your team, suppliers, and customers. Director-only audit view.")}</p>
      </div>
      ...
      <div className="card p-3 flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input
            ...
            placeholder={T("Search filename or description…")}
        ...
        <span className="ml-auto text-xs font-semibold text-ink-500">
          {files.data?.length ?? 0} {T("file")}{(files.data?.length ?? 0) === 1 ? "" : "s"}
        </span>
      </div>
        ...
        <div className="card p-4 text-sm text-red-700 flex items-start gap-2">
          ...
          <div className="flex-1">
            <div className="font-medium">
              {T("Couldn't load files")}{(files.error as any)?.response?.status
      ...
      <div className="card overflow-hidden">
        {files.isLoading ? (
          ...
          <div className="px-5 py-12 text-center text-sm muted">
            {T("No files match your filter.")}</div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-ink-50/60">
              <tr>
                <th className="th">{T("File")}</th>
                <th className="th">{T("Belongs to")}</th>
                <th className="th">{T("Uploaded by")}</th>
                <th className="th">{T("When")}</th>
                <th className="th text-right">{T("Size")}</th>
                <th className="th"></th>
              </tr>
            </thead>
            <tbody>
              {files.data.map((f) => (
                <tr key={f.id} className="tr-hover border-t border-ink-100">
                  ...
                  <td className="td muted">
                    {new Date(f.uploaded_at).toLocaleString(locale())}
                  </td>

    ...
    <form onSubmit={(e) => { e.preventDefault(); setErr(null); save.mutate(); }}
      ...
      <div className="rounded-xl bg-ink-50 border border-ink-100 p-3 text-sm">
        ...
        <div className="muted text-xs">
          {item.sku} {T("· current")}{" "}{item.current_stock} {item.uom}
        </div>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <label className="block">
          <span className="block text-xs font-medium text-ink-600 mb-1">{T("Direction")}</span>
        ...
        <label className="block">
          <span className="block text-xs font-medium text-ink-600 mb-1">{T("Quantity")}</span>
          <input type="number" min={0} step="any" className="input" required
            value={delta} onChange={(e) => setDelta(parseFloat(e.target.value || "0"))} />
        </label>
        <label className="block">
          <span className="block text-xs font-medium text-ink-600 mb-1">{T("Reason")}</span>
          <select className="input" value={reason} onChange={(e) => setReason(e.target.value)}>
            {REASONS.map((r) => <option key={r.value} value={r.value}>{T(r.label)}</option>)}
          </select>
        </label>
        <label className="block">
          <span className="block text-xs font-medium text-ink-600 mb-1">{T("Reference")}</span>
          <input className="input" value={reference} placeholder={T("e.g. WO-2026-0012")}
      ...
      <div className="rounded-lg bg-brand-50 border border-brand-100 px-3 py-2 text-sm">
        {T("After this adjustment:")}{" "}<b className="tabular-nums">{projected} {item.uom}</b>
      </div>

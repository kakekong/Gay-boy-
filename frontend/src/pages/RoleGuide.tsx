...
const ROLES: RoleSection[] = [
  {
    ...
    flow: [
      ...
      { title: "Mark deal Won → finance approves", title_id: "Tandai deal Menang → keuangan menyetujui",
        detail: "Sends a Won request to FINANCE, not the director; the quote flips to Won once they approve. Winning posts the deal to the ledger and opens the project, which is why the desk that lives with the money signs it off.",
    ...
    buttons: [
      ...
      { page: "Customer page", page_id: "Halaman pelanggan", button: "Log activity", button_id: "Catat aktivitas",
        effect: "Record a call or note straight onto the customer's timeline", effect_id: "Catat telepon atau catatan langsung ke linimasa pelanggan" },
      ...
      { page: "Quotation page", page_id: "Halaman penawaran", button: "Log follow-up", button_id: "Catat tindak lanjut",
        effect: "Recorded straight away, with an optional reminder for the next one", effect_id: "Langsung tercatat, dengan pengingat opsional untuk berikutnya" },
      { page: "Quotation page (approved)", page_id: "Halaman penawaran (disetujui)", button: "Mark won", button_id: "Tandai menang",
        effect: "Sends a Won request to finance", effect_id: "Mengirim permintaan Menang ke keuangan" },
      ...
      { page: "My commission", page_id: "Komisi saya", button: "Claim Rp …", button_id: "Klaim Rp …",
        effect: "Your own page, under People in the sidebar. Claim your 2% on a job the customer has paid in full — it goes to the director to agree. The sidebar number is how many are claimable today.",
        effect_id: "Halaman Anda sendiri, di bawah SDM pada sidebar. Klaim 2% Anda atas pekerjaan yang sudah dibayar lunas pelanggan — masuk ke direktur untuk disetujui. Angka di sidebar adalah jumlah yang bisa diklaim hari ini." },
      { page: "My commission", page_id: "Komisi saya", button: "Claim again on Rp …", button_id: "Klaim ulang atas Rp …",
        effect: "On a refused claim — the director's reason sits right above it, and the job is back on offer at whatever has been collected now",
        effect_id: "Pada klaim yang ditolak — alasan direktur ada tepat di atasnya, dan pekerjaannya kembali ditawarkan sesuai uang yang sudah masuk sekarang" },
    ...
    rules: [
      ...
      "Follow-ups you log — on a customer or a quotation — are recorded the moment you save them. The call already happened; the note is the record of it, so it no longer waits on anybody.",
      "Marking a quote Won sends a request to FINANCE — not the director — and it only flips to Won once they approve in /approvals. Winning is the moment the deal becomes money: it posts to the ledger and opens the project, so the desk that carries that signs it off.",
      ...
      "Commission is 2% of what the customer actually paid, and it has its own page — People → My commission. A job only appears there once finance has received the whole invoice: not when the deal is won, not when the goods ship. It reads left to right as where your money is — ready to claim, with the director, agreed and in payroll, paid. Your page shows your claims and nobody else's.",
    ...
    rules_id: [
      ...
      "Menandai penawaran Menang mengirim permintaan ke KEUANGAN — bukan direktur — dan baru jadi Menang setelah disetujui di /approvals. Menang adalah saat deal berubah jadi uang: diposting ke buku besar dan membuka proyek, jadi bagian yang menanggungnya yang menyetujui.",
      ...
      "Komisi adalah 2% dari uang yang benar-benar dibayar pelanggan, dan punya halaman sendiri — SDM → Komisi saya. Sebuah pekerjaan baru muncul di sana setelah keuangan menerima seluruh fakturnya: bukan saat deal menang, bukan saat barang dikirim. Halamannya dibaca kiri ke kanan sebagai posisi uang Anda — siap diklaim, di direktur, disetujui dan masuk penggajian, dibayar. Halaman Anda hanya menampilkan klaim Anda sendiri.",
  ...
  },
  {
    ...
    buttons: [
      ...
      { page: "Purchasing", page_id: "Pembelian", button: "Ask suppliers for a price", button_id: "Tanya harga ke pemasok",
        effect: "Pick the price requests to cost — including ones already priced and approved, marked 'already priced'. Those are the ones you buy against: the deal is won, or the old quote expired. Applying the new quote to one goes to the director as a cost revision rather than changing the cost they approved a margin against.",
        effect_id: "Pilih permintaan harga yang mau dihargai — termasuk yang sudah dihargai dan disetujui, ditandai 'sudah berharga'. Itulah yang jadi dasar pembelian: dealnya menang, atau penawaran lamanya kedaluwarsa. Menerapkan penawaran baru ke sana masuk ke direktur sebagai revisi biaya, bukan mengubah biaya yang marginnya sudah disetujui." },
      { page: "Supplier price request", page_id: "Permintaan harga ke pemasok", button: "Bring this list up to date", button_id: "Perbarui daftar ini",
        effect: "The customer's order moves — a quantity goes up, a line is reworded — and this list keeps saying what the supplier was sent, on purpose. The drift is shown line by line; press this when you are ready to go back to them. What they quoted is kept: their price is per unit, so the line total just follows.",
        effect_id: "Pesanan pelanggan berubah — jumlah naik, uraian diperbaiki — dan daftar ini tetap seperti yang dikirim ke pemasok, memang begitu. Selisihnya ditampilkan per baris; tekan ini saat Anda siap menghubungi mereka lagi. Harga dari pemasok tetap: harganya per satuan, jadi total baris mengikuti." },
      { page: "Purchasing PO detail", page_id: "Detail PO Pembelian", button: "Edit items", button_id: "Ubah item",
        effect: "The money lives with the lines: currency, rate and the lines themselves are set together here, and the header reads back what they say. Switching currency does not convert the prices — it re-denominates them, so change the prices too if the deal changed.",
        effect_id: "Uangnya menyatu dengan barisnya: mata uang, kurs, dan barisnya diatur bersama di sini, dan bagian atas mengikuti. Mengganti mata uang tidak mengonversi harga — hanya mengganti satuannya, jadi ubah harganya juga kalau kesepakatannya berubah." },
  ...
  },
  {
    ...
    buttons: [
      ...
      { page: "Project detail", page_id: "Detail Proyek", button: "Upload delivery proof", button_id: "Unggah bukti kirim",
        effect: "POD / courier slip — finance verifies it before delivery can be confirmed",
        effect_id: "POD / resi kurir — keuangan memverifikasinya sebelum pengiriman bisa dikonfirmasi" },
  ...
  },
  {
    ...
    flow: [
      { title: "Sales marks a deal Won → you sign it off", title_id: "Sales menandai deal Menang → Anda yang menyetujui",
        detail: "Approvals → the mark-won request. Approving is what flips the quote to Won: it posts the deal to the ledger and opens the project, so the invoice schedule starts from your decision.",
        detail_id: "Persetujuan → permintaan tandai-menang. Persetujuan Anda yang mengubah penawaran jadi Menang: memposting deal ke buku besar dan membuka proyek, jadi jadwal faktur dimulai dari keputusan Anda." },
    ...
    buttons: [
      { page: "Approvals", page_id: "Persetujuan", button: "Approve (mark-won)", button_id: "Setujui (tandai-menang)",
        effect: "Flips the quotation to Won — which posts it to the ledger and opens the project. Yours now, not the director's.",
        effect_id: "Mengubah penawaran jadi Menang — memposting ke buku besar dan membuka proyek. Sekarang hak Anda, bukan direktur." },
      ...
      { page: "Finance → Money in", page_id: "Keuangan → Uang masuk", button: "Delete (on a row)", button_id: "Hapus (di baris)",
        effect: "Bin a duplicate invoice where you spot it — two rows, same customer, same figure. Only while nothing has been paid against it; the number opens the invoice so you can check which one you are deleting.",
        effect_id: "Hapus faktur duplikat di tempat Anda melihatnya — dua baris, pelanggan sama, nilai sama. Hanya selama belum ada pembayaran atasnya; nomornya bisa diklik untuk membuka faktur dan memastikan yang mana yang dihapus." },
      { page: "Financial reports", page_id: "Laporan keuangan", button: "Cash on hand (click it)", button_id: "Kas di tangan (klik angkanya)",
        effect: "Correct what an account actually holds. The figure is the sum of the cash + bank balances, so the difference posts as a dated journal entry against opening-balance equity — visible in the general journal and reversible there.",
        effect_id: "Koreksi saldo sebenarnya tiap akun. Angkanya adalah jumlah saldo kas + bank, jadi selisihnya diposting sebagai jurnal bertanggal ke ekuitas saldo awal — terlihat di Jurnal Umum dan bisa dibalik di sana." },
    ...
    rules: [
"Marking a deal Won is yours to approve. Sales files it; you sign it off in Approvals, and approving is what posts the quotation to the ledger and opens the project. The Approvals page is in your sidebar for it — the same queue already carries your DP customer POs.",
      ...
      "You can delete a duplicate/mistaken invoice and its faktur pajak record — but not once a payment has been verified against it. If one was recorded against the wrong invoice, ask the director: reversing a receipt is theirs, not yours. Once they reverse it, nothing stands against the invoice and you can delete it.",
      "Both signatures on a shipment are yours, and yours alone: you approve the invoice AND release the delivery order beside it. The goods and the bill leave together, so the desk reconciling one against the other signs both. Nobody else can — not admin, not a manager, not the director, and not from the Approvals inbox either. The delivery work order can't start until you have released the delivery order.",
      "Your sidebar: Approvals, Customer PO, Projects, Finance, Financial reports, Payment verification, Chart of Accounts, Recent ledgers, Attendance, Chat.",
    ...
    rules_id: [
"Menandai deal Menang adalah persetujuan Anda. Sales mengajukannya; Anda yang menyetujui di Persetujuan, dan persetujuan itulah yang memposting penawaran ke buku besar dan membuka proyek. Halaman Persetujuan kini ada di sidebar Anda — antrean yang sama sudah memuat PO DP pelanggan Anda.",
      ...
      "Sidebar Anda: Persetujuan, PO Pelanggan, Proyek, Keuangan, Laporan keuangan, Verifikasi Pembayaran, Bagan Akun, Ledger Terbaru, Absensi, Chat.",
  ...
  },
  {
    ...
    flow: [
      { title: "Open Approvals first", title_id: "Buka Persetujuan dulu",
        detail: "Quotations, customer POs, stage moves, and Mark-won requests — green-light or reject. Approving Mark-won flips the quote to Won and posts the ledger. Follow-ups are no longer in here; sales record those themselves.",
    ...
    buttons: [
      { page: "Approvals", page_id: "Persetujuan", button: "Approve / Reject", button_id: "Setujui / Tolak",
        effect: "Sign off on every quotation, customer PO, supplier PO, stage move, Mark-won + data change",
      ...
      { page: "Invoice detail", page_id: "Detail faktur", button: "Reverse (beside a payment)", button_id: "Balikkan (di samping pembayaran)",
        effect: "Take a receipt back off the invoice: writes the negative payment + mirror ledger entry, the invoice goes unpaid again and its project off 'closed'. Yours alone — not finance's.",
        effect_id: "Tarik kembali penerimaan dari faktur: menulis pembayaran negatif + entri jurnal balik, faktur kembali belum lunas dan proyeknya turun dari 'closed'. Hanya milik Anda — bukan keuangan." },
    ...
    rules: [
      ...
      "A won quotation is still editable — it is the live deal, with a project running against it. Change a line on it and the price request behind it and the project's order card follow automatically; because winning posted it to the ledger, the posting is reversed and re-posted so the accounts match the new total. Lost and cancelled deals stay locked.",
      "Mark-won is FINANCE's sign-off now, not yours. Winning posts the deal to the ledger and opens the project, so it sits with the desk that lives with both. You can still settle one — you can settle any request in the system — but it is not queued to you and nothing waits on you for it. Follow-ups are not queued either: sales record those as they happen, and you read them on the customer's timeline.",
    ...
    rules_id: [
      ...
      "Penawaran yang sudah Menang tetap bisa diedit — itu deal yang hidup, ada proyek berjalan di belakangnya. Ubah barisnya dan permintaan harga serta kartu pesanan di proyek ikut otomatis; karena Menang sudah memposting ke buku besar, postingnya dibalik dan diposting ulang agar akunnya sesuai total baru. Deal yang kalah atau dibatalkan tetap terkunci.",
      "Mark-won sekarang persetujuan KEUANGAN, bukan Anda. Menang memposting deal ke buku besar dan membuka proyek, jadi diputuskan oleh bagian yang menanggung keduanya. Anda tetap bisa memutuskannya — Anda bisa memutuskan permintaan apa pun — tapi tidak diantrekan ke Anda. Tindak lanjut juga tidak diantrekan: sales mencatatnya saat terjadi dan Anda membacanya di linimasa pelanggan.",

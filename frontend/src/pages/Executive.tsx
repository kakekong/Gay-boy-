    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2">
          <Crown size={22} className="text-amber-500" /> {T("Executive Dashboard")}</h1>
        <p className="text-sm muted">{T("Big-picture view for managers and directors.")}</p>
      </div>
      ...
      <div className="card p-5">
        <div className="font-semibold text-ink-900 mb-3">{T("Top customers by lifetime value")}</div>
        <ul className="space-y-2">
          ...
          {!top.length && <li className="muted text-sm">{T("No data yet.")}</li>}
        </ul>
      </div>
    </div>
  );
